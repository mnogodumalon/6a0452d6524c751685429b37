...
interface VerknuepfungenDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Verknuepfungen['fields']) => Promise<void>;
  defaultValues?: Verknuepfungen['fields'];
  eintraegeList: Eintraege[];
  enablePhotoScan?: boolean;
  enablePhotoLocation?: boolean;
}

export function VerknuepfungenDialog({ open, onClose, onSubmit, defaultValues, eintraegeList, enablePhotoScan = true, enablePhotoLocation = true }: VerknuepfungenDialogProps) {
  const [fields, setFields] = useState<Partial<Verknuepfungen['fields']>>({});
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const clean = cleanFieldsForApi({ ...fields }, 'verknuepfungen');
      await onSubmit(clean as Verknuepfungen['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      contextParts.push(`<available-records field="eintrag_referenz" entity="Einträge">\n${JSON.stringify(eintraegeList.map(r => ({ record_id: r.record_id, ...r.fields })), null, 2)}\n</available-records>`);
      if (usePersonalInfo) {
        try {
      ...
      setFields(prev => {
        ...
        const applookupKeys = new Set<string>(["eintrag_referenz"]);
        for (const [k, v] of Object.entries(raw)) {
          if (applookupKeys.has(k)) continue;
          if (v != null) merged[k] = v;
        }
        const eintrag_referenzName = raw['eintrag_referenz'] as string | null;
        if (eintrag_referenzName) {
          const eintrag_referenzMatch = eintraegeList.find(r => matchName(eintrag_referenzName!, [String(r.fields.titel ?? '')]));
          if (eintrag_referenzMatch) merged['eintrag_referenz'] = createRecordUrl(APP_IDS.EINTRAEGE, eintrag_referenzMatch.record_id);
        }
        return merged as Partial<Verknuepfungen['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'Verknüpfungen bearbeiten' : 'Verknüpfungen hinzufügen';
      ...
      <DialogContent className="max-w-lg max-h-[92vh] flex flex-col overflow-hidden p-0 gap-0">
        ...
        <form onSubmit={handleSubmit} className="flex flex-1 flex-col min-h-0">
          ...
          <div className="space-y-1.5">
            <Label htmlFor="eintrag_referenz">Verknüpfter Eintrag</Label>
            <Combobox
              id="eintrag_referenz"
              items={eintraegeList.map(r => ({
                id: r.record_id,
                label: String(r.fields.titel ?? r.record_id),
              }))}
              value={extractRecordId(fields.eintrag_referenz)}
              onChange={id => setFields(f => ({ ...f, eintrag_referenz: id ? createRecordUrl(APP_IDS.EINTRAEGE, id) : undefined }))}
