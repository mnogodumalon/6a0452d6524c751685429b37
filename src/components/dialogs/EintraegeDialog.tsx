...
interface EintraegeDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Eintraege['fields']) => Promise<void>;
  defaultValues?: Eintraege['fields'];
  enablePhotoScan?: boolean;
  enablePhotoLocation?: boolean;
}

export function EintraegeDialog({ open, onClose, onSubmit, defaultValues, enablePhotoScan = true, enablePhotoLocation = true }: EintraegeDialogProps) {
  const [fields, setFields] = useState<Partial<Eintraege['fields']>>({});
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const clean = cleanFieldsForApi({ ...fields }, 'eintraege');
      await onSubmit(clean as Eintraege['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      setFields(prev => {
        ...
        return merged as Partial<Eintraege['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'Einträge bearbeiten' : 'Einträge hinzufügen';
