...
export default function EintraegePage() {
  const [records, setRecords] = useState<Eintraege[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<Eintraege | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<Eintraege | null>(null);
  const [viewingRecord, setViewingRecord] = useState<Eintraege | null>(null);
  ...
  async function loadData() {
    setLoading(true);
    try {
      setRecords(await LivingAppsService.getEintraege());
    } finally {
      setLoading(false);
    }
  }

  async function handleCreate(fields: Eintraege['fields']) {
    await LivingAppsService.createEintraegeEntry(fields);
    await loadData();
    setDialogOpen(false);
  }

  async function handleUpdate(fields: Eintraege['fields']) {
    if (!editingRecord) return;
    await LivingAppsService.updateEintraegeEntry(editingRecord.record_id, fields);
    await loadData();
  ...
  async function handleDelete() {
    ...
    await LivingAppsService.deleteEintraegeEntry(deleteTarget.record_id);
  ...
  return (
    <PageShell
      title="Einträge"
      subtitle={`${records.length} Einträge im System`}
      action={
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={4} className="text-center py-16 text-muted-foreground">
                  {search ? 'Keine Ergebnisse gefunden.' : 'Noch keine Einträge. Jetzt hinzufügen!'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
