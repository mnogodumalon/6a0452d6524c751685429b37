...
import { EintraegeViewDialog } from '@/components/dialogs/EintraegeViewDialog';
import { VerknuepfungenDialog } from '@/components/dialogs/VerknuepfungenDialog';
import { VerknuepfungenViewDialog } from '@/components/dialogs/VerknuepfungenViewDialog';
...
const VERKNUEPFUNGEN_FIELDS = [
  { key: 'eintrag_referenz', label: 'Verknüpfter Eintrag', type: 'applookup/select', targetEntity: 'eintraege', targetAppId: 'EINTRAEGE', displayField: 'titel' },
...
const ENTITY_TABS = [
  { key: 'eintraege', label: 'Einträge', pascal: 'Eintraege' },
  { key: 'verknuepfungen', label: 'Verknüpfungen', pascal: 'Verknuepfungen' },
] as const;
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('eintraege');
  const [selectedIds, setSelectedIds] = useState<Record<EntityKey, Set<string>>>(() => ({
    'eintraege': new Set(),
    'verknuepfungen': new Set(),
  }));
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'eintraege': return (data as any).eintraege as Eintraege[] ?? [];
      case 'verknuepfungen': return (data as any).verknuepfungen as Verknuepfungen[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getLookupLists = useCallback((entity: EntityKey) => {
    ...
    switch (entity) {
      case 'verknuepfungen':
        lists.eintraegeList = (data as any).eintraege ?? [];
        break;
    }
    return lists;
  }, [data]);
  ...
  const getApplookupDisplay = useCallback((entity: EntityKey, fieldKey: string, url?: unknown) => {
    ...
    if (entity === 'verknuepfungen' && fieldKey === 'eintrag_referenz') {
      const match = (lists.eintraegeList ?? []).find((r: any) => r.record_id === id);
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'eintraege': return EINTRAEGE_FIELDS;
      case 'verknuepfungen': return VERKNUEPFUNGEN_FIELDS;
      default: return [];
    }
  }, []);
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'eintraege': return {
        create: (fields: any) => LivingAppsService.createEintraegeEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateEintraegeEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteEintraegeEntry(id),
      };
      case 'verknuepfungen': return {
        create: (fields: any) => LivingAppsService.createVerknuepfungenEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateVerknuepfungenEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteVerknuepfungenEntry(id),
      };
      default: return null;
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'eintraege' || dialogState?.entity === 'eintraege') && (
        <EintraegeDialog
          open={createEntity === 'eintraege' || dialogState?.entity === 'eintraege'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'eintraege' ? handleUpdate : (fields: any) => handleCreate('eintraege', fields)}
          defaultValues={dialogState?.entity === 'eintraege' ? dialogState.record?.fields : undefined}
          enablePhotoScan={AI_PHOTO_SCAN['Eintraege']}
          enablePhotoLocation={AI_PHOTO_LOCATION['Eintraege']}
        />
      )}
      {(createEntity === 'verknuepfungen' || dialogState?.entity === 'verknuepfungen') && (
        <VerknuepfungenDialog
          open={createEntity === 'verknuepfungen' || dialogState?.entity === 'verknuepfungen'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'verknuepfungen' ? handleUpdate : (fields: any) => handleCreate('verknuepfungen', fields)}
          defaultValues={dialogState?.entity === 'verknuepfungen' ? dialogState.record?.fields : undefined}
          eintraegeList={(data as any).eintraege ?? []}
          enablePhotoScan={AI_PHOTO_SCAN['Verknuepfungen']}
          enablePhotoLocation={AI_PHOTO_LOCATION['Verknuepfungen']}
        />
      )}
      {viewState?.entity === 'eintraege' && (
        <EintraegeViewDialog
          open={viewState?.entity === 'eintraege'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'eintraege', record: r }); }}
        />
      )}
      {viewState?.entity === 'verknuepfungen' && (
        <VerknuepfungenViewDialog
          open={viewState?.entity === 'verknuepfungen'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'verknuepfungen', record: r }); }}
          eintraegeList={(data as any).eintraege ?? []}
        />
      )}
