...
import type { Eintraege, Verknuepfungen, CreateEintraege, CreateVerknuepfungen } from '@/types/app';
...
export class LivingAppsService {
  // --- EINTRAEGE ---
  static async getEintraege(): Promise<Eintraege[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.EINTRAEGE}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Eintraege[];
    return enrichLookupFields(records, 'eintraege');
  }
  static async getEintraegeEntry(id: string): Promise<Eintraege | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.EINTRAEGE}/records/${id}`);
    const record = { record_id: data.id, ...data } as Eintraege;
    return enrichLookupFields([record], 'eintraege')[0];
  }
  static async createEintraegeEntry(fields: CreateEintraege) {
    return callApi('POST', `/apps/${APP_IDS.EINTRAEGE}/records`, { fields: cleanFieldsForApi(fields as any, 'eintraege') });
  }
  static async updateEintraegeEntry(id: string, fields: Partial<CreateEintraege>) {
    return callApi('PATCH', `/apps/${APP_IDS.EINTRAEGE}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'eintraege') });
  }
  static async deleteEintraegeEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.EINTRAEGE}/records/${id}`);
  }

  // --- VERKNUEPFUNGEN ---
  static async getVerknuepfungen(): Promise<Verknuepfungen[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.VERKNUEPFUNGEN}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Verknuepfungen[];
    return enrichLookupFields(records, 'verknuepfungen');
  }
  static async getVerknuepfungenEntry(id: string): Promise<Verknuepfungen | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.VERKNUEPFUNGEN}/records/${id}`);
    const record = { record_id: data.id, ...data } as Verknuepfungen;
    return enrichLookupFields([record], 'verknuepfungen')[0];
  }
  static async createVerknuepfungenEntry(fields: CreateVerknuepfungen) {
    return callApi('POST', `/apps/${APP_IDS.VERKNUEPFUNGEN}/records`, { fields: cleanFieldsForApi(fields as any, 'verknuepfungen') });
  }
  static async updateVerknuepfungenEntry(id: string, fields: Partial<CreateVerknuepfungen>) {
    return callApi('PATCH', `/apps/${APP_IDS.VERKNUEPFUNGEN}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'verknuepfungen') });
  }
  static async deleteVerknuepfungenEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.VERKNUEPFUNGEN}/records/${id}`);
  }
